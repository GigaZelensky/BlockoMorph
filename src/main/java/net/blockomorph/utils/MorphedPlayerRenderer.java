...
public class MorphedPlayerRenderer {
            ...
            try {
                ...
                if (renderer != null) {
                    ...
                    if (k > -1 && k < 10) {
                        ...
                        VertexConsumer vertexconsumer = new SheetedDecalTextureGenerator(crumblBuffer.getBuffer(ModelBakery.DESTROY_TYPES.get(k)), posestack$pose.pose(), posestack$pose.normal(), 1.0F);
                        src = (p_234298_) -> {
    ...
    private void renderBreak(int k, BlockState blockstate, AbstractClientPlayer player, PoseStack posestack, MultiBufferSource buffer, ModelData data, BlockInPlayer2 block) {
        ...
        if (k > -1 && k < 10) {
            VertexConsumer vertexconsumer1 = new SheetedDecalTextureGenerator(crumblBuffer.getBuffer(ModelBakery.DESTROY_TYPES.get(k)), posestack$pose1.pose(), posestack$pose1.normal(), 1.0F);
            BlockPos offset = this.getPosForOffset(block);
